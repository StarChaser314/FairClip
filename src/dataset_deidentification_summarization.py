...
def preprocess(model_name, dataset, mode, dataset_path):
    if model_name == "med42":
        tokenizer = AutoTokenizer.from_pretrained(
            "m42-health/med42-70b", use_fast=False, cache_dir=CACHE_DIR
        )  # llama-2 model fine-tuned for medical domain
        ...
        tokenizer = AutoTokenizer.from_pretrained(
            "Writer/palmyra-med-20b", use_fast=False, cache_dir=CACHE_DIR
        )
        model = AutoModelForCausalLM.from_pretrained(
        ...
        tokenizer = transformers.LlamaTokenizer.from_pretrained(
            "axiong/PMC_LLaMA_13B", cache_dir=CACHE_DIR
        )
        model = transformers.LlamaForCausalLM.from_pretrained(
            "axiong/PMC_LLaMA_13B", cache_dir=CACHE_DIR, device_map="auto"
        )
    ...
    for prompt in prompts:
        ...
        for i in range(len(dataset)):
            file_name = dataset[i]
            orig_note = (
                np.load(os.path.join(dataset_path, file_name))["note"].item().strip()
            )
            ...
            if model_name in ["med42"]:
                ...
                processed_note = processed_note[
                    processed_note.find(note) + len(note) :
                ].strip("</s>")
            elif model_name == "palmyra-med":
                ...
                processed_note = tokenizer.decode(
...
def merge_all_models_summaries():
    ...
    csv_files = [
        "flan-t5-base-samsum_summarized_notes.csv",
        "bart-large-samsum_summarized_notes.csv",
        "bart-large-cnn-samsum_summarized_notes.csv",
        "pmc-llama_summarize.csv",
        "palmyra-med_summarize.csv",
        "med42_summarize.csv",
        "gpt-4_summarize.csv",
    ]
    model_names = [
    ...
    for csv_file, model_name in zip(csv_files, model_names):
        ...
        df.rename(
            columns={"Summarized Note": f"Summarized Note ({model_name})"}, inplace=True
        )
        if merged_df.empty:
            merged_df = df
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        type=str,
        choices=["med42", "palmyra-med", "pmc-llama", "gpt-3.5-turbo", "gpt-4"],
    )
    parser.add_argument("--openai_key", type=str)
    args = parser.parse_args()

    openai.api_key = args.openai_key
    dataset_path = "FUNDUS_Dataset/FairVLMed/Testing"
