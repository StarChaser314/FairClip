...
parser.add_argument(
    ...
    type=float,
    metavar="W",
    help="weight decay (default: 6e-5)",
    dest="weight_decay",
)
...
parser.add_argument(
    "--text_source", default="note", type=str, help="options: note | label"
)
parser.add_argument("--perf_file", default="", type=str)
parser.add_argument(
    "--model_arch", default="vit-b16", type=str, help="options: vit-b16 | vit-l14"
)
parser.add_argument("--pretrained_weights", default="", type=str)
parser.add_argument(
    "--attribute", default="race", type=str, help="race|gender|ethnicity|language"
)
...
if __name__ == "__main__":
    ...
    if args.perf_file != "":
        if not os.path.exists(best_global_perf_file):
            ...
            dpd_head_str += (
                ", ".join([f"dpd_attr{x}" for x in range(len(groups_in_attrs))]) + ", "
            )
            eod_head_str += (
                ", ".join([f"eod_attr{x}" for x in range(len(groups_in_attrs))]) + ", "
            )
            esacc_head_str += (
                ", ".join([f"esacc_attr{x}" for x in range(len(groups_in_attrs))])
                + ", "
            )
            esauc_head_str += (
                ", ".join([f"esauc_attr{x}" for x in range(len(groups_in_attrs))])
                + ", "
            )
            ...
            group_disparity_head_str += (
                ", ".join(
                    [
                        f"std_group_disparity_attr{x}, max_group_disparity_attr{x}"
                        for x in range(len(groups_in_attrs))
                    ]
                )
                + ", "
            )
    ...
    for epoch in range(1):
        ...
        if args.result_dir is not None:
            np.savez(
                os.path.join(args.result_dir, f"pred_gt_ep{epoch:03d}.npz"),
                val_pred=all_probs,
                val_gt=all_labels,
                val_attr=all_attrs,
            )
        ...
        for ii in range(len(eval_aucs_by_attrs)):
            for iii in range(len(eval_aucs_by_attrs[ii])):
                logger.logkv(
                    f"eval_auc_attr{ii}_group{iii}",
                    round(eval_aucs_by_attrs[ii][iii], 4),
                )

        for ii in range(len(between_group_disparity)):
            logger.logkv(
                f"eval_auc_attr{ii}_std_group_disparity",
                round(between_group_disparity[ii][0], 4),
            )
            logger.logkv(
                f"eval_auc_attr{ii}_max_group_disparity",
                round(between_group_disparity[ii][1], 4),
            )
        ...
        if os.path.exists(best_global_perf_file):
            with open(best_global_perf_file, "a") as f:
                ...
                for i in range(len(best_auc_groups)):
                    auc_head_str += (
                        ", ".join([f"{x:.4f}" for x in best_auc_groups[i]]) + ", "
                    )

                group_disparity_str = ""
                for i in range(len(best_between_group_disparity)):
                    group_disparity_str += (
                        ", ".join([f"{x:.4f}" for x in best_between_group_disparity[i]])
                        + ", "
                    )
