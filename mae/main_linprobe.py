...
def get_args_parser():
    ...
    parser.add_argument(
        "--vl_feats_type", type=str, required=True, choices=["image", "multimodal"]
    )
    parser.add_argument(
        "--blip_feats_select",
        type=str,
...
def main(args):
    ...
    dataset_train = FUNDUS_Dataset(
        args.data_path, "train", transform_train, text_processor, args.summary_type
    )
    dataset_val = FUNDUS_Dataset(
        args.data_path, "val", transform_val, text_processor, args.summary_type
    )
    dataset_test = FUNDUS_Dataset(
        args.data_path, "test", transform_test, text_processor, args.summary_type
    )
    print(dataset_train)
    print(dataset_val)
    print(dataset_test)
    ...
    if args.model_type in ["vit", "mae"]:
        if args.model_type == "vit":
            imagenet_init_model = timm.models.vision_transformer.vit_large_patch16_224(
                pretrained=True
            )
    ...
    if args.eval:
        ...
        print(
            f"AUC of the network on the {len(dataset_test)} test images: {test_stats['overall_auc']:.1f}%"
        )
        exit(0)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % 10 == 0:
            ...
            print(
                f"AUC of the network on the {len(dataset_val)} val images: {val_stats['overall_auc']}"
            )
            print(
