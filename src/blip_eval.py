...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        choices=["clip", "pmc-clip", "fundus-clip", "fundus-mae"],
    )
    parser.add_argument(
        "--vl_type", type=str, required=True, choices=["clip", "blip", "blip2"]
    )
    parser.add_argument(
        "--prompt", type=str, required=True, default="A picture of "
    )  # a photo of a -- CLIP prompt
    parser.add_argument(
        "--eval_type", type=str, required=True, choices=["zero_shot", "linear_probe"]
    )
    parser.add_argument(
        "--summary_type",
        type=str,
        required=True,
        choices=["original", "pmc-llama", "med42", "gpt-3.5-turbo", "gpt-4"],
    )
...
def zero_shot_blip(args):
    ...
    dataset = FUNDUS_Dataset(
        dataset_dir, subset, vis_processor, text_processor, summary_type
    )
    text_processor = BlipCaptionProcessor(
        prompt=prompt, max_words=50
    )  # for creating prompts from class names

    if vl_type == "clip":
        model, vis_processors, text_processor = load_model_and_preprocess(
            "clip_feature_extractor", model_type="ViT-L-14", is_eval=True, device=device
        )
    elif vl_type == "blip":
    ...
    for idx in range(len(dataset)):
        ...
        if vl_type == "clip":
            ...
            sims = (image_features @ text_features.t()).max(dim=1).values[
                0
            ] / model.temp
    ...
    ) = evalute_comprehensive_perf(all_probs, all_labels, all_attrs.T)
    ...
    accuracy = np.mean((labels == predictions).astype(float)) * 100.0
    ...
    with torch.no_grad():
        for batch in tqdm(DataLoader(dataset, batch_size=100)):
            ...
            if vl_type == "clip":
                ...
                image_features = model.extract_features(
                    {"image": images.to(device)}, mode="image"
                ).image_embeds[:, 0]
            elif vl_type == "blip2":
                image_features = model.extract_features(
                    {"image": images.to(device)}, mode="image"
                ).image_embeds.flatten(1)
...
def linear_probe_blip(args):
    ...
    vis_processor = BlipImageEvalProcessor(image_size=224, mean=None, std=None)
    text_processor = BlipCaptionProcessor(prompt="", max_words=50)  # default settings
    train = FUNDUS_Dataset(
        dataset_dir, "train", vis_processor, text_processor, summary_type
    )
    test = FUNDUS_Dataset(
        dataset_dir, "test", vis_processor, text_processor, summary_type
    )
    text_processor = BlipCaptionProcessor(
        prompt=prompt, max_words=50
    )  # for creating prompts from class names
    ...
    train_features, train_labels, train_attrs = get_features_blip(
        train, model, device, vl_type
    )
    test_features, test_labels, test_attrs = get_features_blip(
        test, model, device, vl_type
    )
    ...
    (
        ...
        eval_eods,
        between_group_disparity,
    ) = evalute_comprehensive_perf(
        classifier.predict_proba(test_features)[:, 1], test_labels, test_attrs.T
    )
