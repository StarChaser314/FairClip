...
class FUNDUS_Dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        if self.summary_type == "original":
            self.summary_file = pd.read_csv(
                os.path.join(dataset_dir, f"original_notes.csv")
            )
        else:
            self.summary_file = pd.read_csv(
                os.path.join(dataset_dir, f"{self.summary_type}_summarized_notes.csv")
            )
    ...
    def __getitem__(self, idx):
        ...
        note = str(
            self.summary_file.loc[self.summary_file["filename"] == self.files[idx]]
            .iloc[:, 2]
            .item()
        ).strip()
        ...
        return {
            ...
            "attributes": torch.tensor(
                [
                    int(data["race"].item()),
                    int(data["gender"].item()),
                    int(data["ethnicity"].item()),
                    int(data["language"].item()),
                ]
            ),
        }
