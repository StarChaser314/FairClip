...
def find_all_files(folder, suffix="npz"):
    files = [
        ...
        and os.path.join(folder, f).endswith(suffix)
    ]
    return files
...
def count_number_of_groups(input_dataset):
    ...
    instances_on_language = []
    instances_on_marital = []
    for file in input_dataset.files:
        ...
        instances_on_language.append(data["language"].item())
        instances_on_marital.append(data["maritalstatus"].item())
    ...
    _, numbers_of_language = np.unique(instances_on_language, return_counts=True)
    _, numbers_of_marital = np.unique(instances_on_marital, return_counts=True)
    return (
        numbers_of_race,
        numbers_of_gender,
        numbers_of_ethnicity,
        numbers_of_language,
        numbers_of_marital,
    )
...
class fair_vl_med_dataset(torch.utils.data.Dataset):
    def __init__(
        ...
        subset="Training",
        text_source="note",
        summarized_note_file=None,
        ruleout_unknown=False,
    ):
        ...
        if (
            ...
            and self.text_source == "note"
            and summarized_note_file != ""
        ):
        ...
        if subset != "Training" or self.ruleout_unknown:
            ...
            for file in self.files:
                ...
                if (
                    data["race"].item() != -1
                    and data["gender"].item() != -1
                    and data["ethnicity"].item() != -1
                    and data["language"].item() != -1
                    and data["maritalstatus"].item() != -1
                ):
    ...
    def __getitem__(self, idx):
        ...
        marital = int(data["maritalstatus"].item())
        # merge all labels together into a single tensor
        label_and_attributes = torch.tensor(
            [glaucoma_label, race, gender, hispanic, language, marital]
        )
...
def multiclass_demographic_parity(pred_prob, y, attrs):
    ...
    for i in range(pred_one_hot.shape[1]):
        tmp_score = demographic_parity_difference(
            pred_one_hot[:, i], gt_one_hot[:, i], sensitive_features=attrs
        )
...
def multiclass_demographic_parity_(pred_prob, y, attrs):
    ...
    for i in range(pred_prob.shape[1]):
        ...
        for j in attrs_set:
            idx = attrs == j
            tmp = np.abs(
                tmp_preds.mean().item() - tmp_preds[idx].mean().item()
            ) + np.abs(tmp_not_preds.mean().item() - tmp_not_preds[idx].mean().item())
...
def equity_scaled_accuracy(output, target, attrs, alpha=1.0):
    es_acc = 0
    ...
    for one_attr in np.unique(attrs).astype(int):
        ...
        if len(pred_group.shape) >= 2:
            ...
            acc = (
                np.sum((pred_group >= 0.5).astype(float) == gt_group)
                / gt_group.shape[0]
            )
...
def equity_scaled_AUC(output, target, attrs, alpha=1.0, num_classes=2):
    es_auc = 0
    tmp = 0
    identity_wise_perf = []
    identity_wise_num = []
    ...
    if num_classes == 2:
        ...
        overall_auc = roc_auc_score(
            y_onehot, output, average="macro", multi_class="ovr"
        )
    ...
    for one_attr in np.unique(attrs).astype(int):
        ...
        if num_classes == 2:
            ...
            group_auc = roc_auc_score(
                y_onehot, pred_group, average="macro", multi_class="ovr"
            )
...
def evalute_perf_by_attr(preds, gts, attrs=None, num_classes=2):
    ...
    for i in range(attrs.shape[0]):
        ...
        if num_classes == 2:
            ...
            dpr = demographic_parity_ratio(gts, pred_labels, sensitive_features=attr)
            eod = equalized_odds_difference(gts, pred_labels, sensitive_features=attr)
            eor = equalized_odds_ratio(gts, pred_labels, sensitive_features=attr)
        elif num_classes > 2:
...
def evalute_comprehensive_perf_(preds, gts, attrs=None, num_classes=2):
    ...
    for i in range(attrs.shape[0]):
        ...
        aucs_by_attrs.append(aucs_by_group)
        std_disparity, max_disparity = compute_between_group_disparity_half(
            aucs_by_group, overall_auc
        )
        between_group_disparity.append([std_disparity, max_disparity])
...
def compute_between_group_disparity(auc_list, overall_auc):
    return np.std(auc_list) / overall_auc, (
        np.max(auc_list) - np.min(auc_list)
    ) / overall_auc


def compute_between_group_disparity_half(auc_list, overall_auc):
    return np.std(auc_list) / np.abs(overall_auc - 0.5), (
        np.max(auc_list) - np.min(auc_list)
    ) / np.abs(overall_auc - 0.5)
